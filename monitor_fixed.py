...
def discover_products(source):
    ...
    excluded_words = [
        "/cart", "/account", "/login", "/stores", "/search",
        "/help", "/about", "/vote", "/ownership", "/shipping",
        "/returns", "/privacy", "/terms", "/contact", "/careers",
        "/blog", "/events", "/community", "/membership", "/gift",
        "/wishlist", "/size", "/filter", "/sort",
    ]

    excluded_extensions = (
        ".jpg", ".jpeg", ".png", ".gif", ".webp", ".avif",
        ".svg", ".ico", ".pdf", ".mp4", ".webm", ".zip",
    )
    ...
    for url in links:
        ...
        # 排除明显的分类页。
        if path.endswith(("/mens", "/men", "/c/mens", "/c/men", "/shop/mens", "/shop/men")):
            continue

        if not any(brand in low for brand in brand_words):
            continue

        product_path = any(
            keyword in path
            for keyword in ("/product/", "/products/", "/item/", "/p/")
        )

        # 部分官网商品页没有 /product/，因此允许较长的商品 slug。
        if not product_path and len(last_part) < 5:
            continue
        ...
        name = last_part.replace("-", " ").replace("_", " ").strip()
        if not name:
            continue
...
def check_product(product, history):
    ...
    current_product = build_current_product(product, data)
    if current_product is None:
        print("没有获取到有效价格")
        return

    current_price = current_product["current_price"]
    current_discount = current_product["discount"]
    current_sale = current_discount > 0
    key = product["url"]
    ...
    if previous:
        ...
        previous_sale = previous_discount > 0

        price_changed_down = (
            previous_price is not None and current_price < previous_price
        )
        entered_sale = current_sale and not previous_sale
        crossed_discount_threshold = (
            previous_discount < MIN_DISCOUNT
        ...
        # 2. 商品从非促销进入促销 -> 提醒
        # 3. 折扣首次达到 30% -> 提醒
        if price_changed_down and current_discount >= MIN_DISCOUNT:
            should_push = True
            push_reason.append("价格下降且折扣达到30%+")

        if entered_sale:
            should_push = True
            push_reason.append("商品进入促销")

        if crossed_discount_threshold:
            should_push = True
            push_reason.append("折扣达到30%+")

        # 相同价格、相同折扣、促销状态没有变化时，不重复推送。
        if (
            previous_price is not None
            and current_price == previous_price
            and current_discount == previous_discount
            and not entered_sale
            and not crossed_discount_threshold
        ):
            should_push = False

    if should_push:
        message = build_telegram_message(current_product, previous)
        if push_reason:
            message = message.replace(
                "🔥 户外商品降价提醒",
                "🔥 户外商品价格/促销提醒\n\n"
                f"📢 触发原因：{'；'.join(push_reason)}",
                1,
            )
    ...
    print(f"折扣：{current_discount:.1f}%")
